...
class CVAE(nn.Module):
    ...
    def __init__(self, *,
        ...
        self.fc1 = nn.Linear(self.input_size + self.num_classes, self.units)
        ...
        self.fc4 = nn.Linear(self.latent_size + self.num_classes, self.units)
        self.fc5 = nn.Linear(self.units, self.units)
        self.fc6 = nn.Linear(self.units, self.units)
        self.layer_output = nn.Linear(self.units, self.input_size)
    ...
    def generation_model(self, z, c): # P(x|z, c)
        ...
        h4 = self.relu(self.fc4(inputs))
        h5 = self.relu(self.fc5(h4))
        h6 = self.relu(self.fc6(h5))

        x_hat = self.sigmoid(self.layer_output(h6))
...
def loss_function(x_hat, x, mu, logvar):
    ...
    DKL = -0.5 * torch.sum(1 + logvar - mu**2 - logvar.exp())

    loss = (MSE + DKL) / x.shape[0]
